...
import pandas as pd
import json
from app.core.process_data import process_data_av, process_data_granular
...
async def process_all_av_files(df_s):
    ...
    for x in os.listdir(JSON_PATH_AV):
        if x.endswith('.json'):
            ...
            json_file = os.path.join(JSON_PATH_AV, x)
...
def omega_report(file):
    try:
        ...
        df_s = df_s.drop(COLS_TO_DROP, axis=1)
        df_g = df_g.drop(COLS_TO_DROP_GRANULAR, axis=1)
        ...
        with open(COMPONENT_GROUPS_PATH, 'r') as json_file:
